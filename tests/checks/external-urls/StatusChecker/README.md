...
```js
// Simple example
const statuses = await getUrlStatusesInParallel([ 'https://anonseclab.org', /* ... */ ]);
...
```js
// Simple example
const status = await getUrlStatus('https://anonseclab.org');
...
```js
const status = await fetchFollow('https://anonseclab.org', 1000 /* timeout in milliseconds */);
console.log(`Status code: ${status.code}`); 
```
